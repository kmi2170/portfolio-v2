...
const Skills = ({ data: skills }: { data: DataSkills[] }) => {
  ...
  return (
    <Wrapper>
      ...
      <Container>
        {skills.length && skillSet?.languages.map((lang) => {
          const skillIcon = skills.filter((skill) => skill.iconName === lang.iconName)
      ...
      <Container>
        {skills.length && skillSet?.tools.map((tool) => {
          const skillIcon = skills.filter((skill) => skill.iconName === tool.iconName)
          const url = urlFor(skillIcon[0]?.icons?.original)
          const iconName = skillIcon[0].iconName
      ...
      <Container>
        {skills.length && skillSet?.databases.map((database) => {
          const skillIcon = skills.filter((skill) => skill.iconName === database.iconName)
          const url = urlFor(skillIcon[0].icons.original)
          const iconName = skillIcon[0].iconName
      ...
      <Container>
        {skills.length && skillSet?.others.map((other) => {
          return (
            <motion.div
              key={other.name}
              whileInView={{ opacity: [0, 1] }}
